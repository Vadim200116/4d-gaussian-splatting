...
class GaussianModel:
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        fused_point_cloud = torch.tensor(np.asarray(pcd.points), dtype=torch.float32).float().cuda()
        fused_color = RGB2SH(torch.tensor(np.asarray(pcd.colors), dtype=torch.float32).float().cuda())
